...
class TestRPCCallsMixin(object):
    ...
    def get_rpc(self, responses):
        """
        Get an RPC object set up to communicate with a thread which will respond to
        each packet with the corresponding box in RESPONSES, remaining silent if that is
        None.  All boxes received are placed in self.received_boxes.
        """
    ...
    def test_single_call_remote(self):
        ...
        self.failUnlessEqual(rpc.call_remote('jump', howhigh='very'), { 'sir' : 'yes, sir' })
    ...
    def test_multiple_call_remote(self):
        ...
        self.failUnlessEqual(rpc.call_remote('stand', dir='up'), { 'position' : 'standing' })
        self.failUnlessEqual(rpc.call_remote('sit', dir='down'), { 'position' : 'sitting' })
        self.failUnlessEqual(self.received_boxes, [
    ...
    def test_multiple_call_remote_noanswer(self):
        ...
        self.failUnlessEqual(rpc.call_remote_no_answer('step', dir='around'), None)
        self.failUnlessEqual(rpc.call_remote('step', dir='down'), { 'position' : 'abdicated' })
        self.failUnlessEqual(rpc.call_remote_no_answer('step', dir='in'), None)
        self.failUnlessEqual(rpc.call_remote('step', dir='up'), { 'position' : 'accepted' })
        self.failUnlessEqual(self.received_boxes, [
